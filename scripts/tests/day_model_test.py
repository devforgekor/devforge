...
Inference container에 14B 올려서 MCP context 포함 full extract 1회 + MiniCheck verify.
...
from lib.test_common import call_llm, parse_llm_json, test_complete, test_setup
...
GT = [
    {
        ...
        "facts": [
            ...
            "DB connection pool exhaustion이 근본 원인",
        ],
        "mcp_facts": [
            "Repository: devforge/server, Branch: main, Last deploy: 2026-06-08",
            "PostgreSQL 16, pg_trgm enabled, JSONB available",
        ],
    },
    {
        "id": "security_leak",
...
def restart_inference():
    ...
    with open("/opt/ai_data/scripts/current-mode-inference.env", "w") as f:
        f.write(env_content)
    print("  Env written. Stopping inference...", flush=True)
    _podman_stop_inference()
    time.sleep(3)
    print("  Starting inference with 14B Q8_0...", flush=True)
...
def run_test():
    ...
    print("  Inference solo, 4 cores, 5 test cases", flush=True)
    print("=" * 70, flush=True)

    # Restart inference with 14B
    print("\n── Inference 14B Q8_0 swap ──", flush=True)
    ok = restart_inference()
    if not ok:
        print("[FAIL] Inference failed to start", flush=True)
        return
    print(flush=True)
    ...
    for i, gt in enumerate(GT):
        ...
        print(
            f"  14B extract: {extract_s}s, {len(findings)} findings, {resp['tps']}t/s", flush=True
        )
    ...
    for r in results:
        ...
        print(
            f"  {r['id']:15s} {r['score']:3d}/100 {bar}  {r['findings']}f/{r['hallu']}h  {r['total_s']:5.0f}s"
        )
