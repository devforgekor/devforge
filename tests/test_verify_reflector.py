#!/usr/bin/env python3
# Status: experimental
# Path: tests/test_verify_reflector.py — pytest
...
print("=" * 60)
print("14B Q8_0 Verify Test (vs 30B)")
...
v30_res = v30
print(f"\n{'='*60}")
print(f"비교: 30B vs 14B Q8_0")
print(f"{'='*60}")
print(f"\n{'항목':<30} {'30B':<25} {'14B Q8_0':<25}")
print(f"{'-'*80}")
...
print(f"\n[verification_items] 30B: {len(v30_items)} | 14B Q8_0: {len(q8_items)}")
...
# Save
result_file = f"{EXPER_DIR}/exp_verify_14b_q8_comparison.json"
out = {
    "meta": {"test": "30B vs 14B Q8_0 verify comparison", "created_at": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())},
    ...
    "14b_q8_0": {"final_verdict": result.get("final_verdict"), "action": result.get("action"), "confidence": result.get("confidence"), "summary": result.get("summary"), "reasoning": result.get("reasoning"), "verification_items": q8_items, "elapsed_ms": elapsed * 1000, "usage": usage},
}
