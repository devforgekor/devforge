...
EXP_PRIMARY = os.path.join(EXPER_DIR, "exp_30b_r1_norubric_r1_norubric.json")
OUTPUT_BASELINE = os.path.join(EXPER_DIR, "exp_3b_verify.json")

with open(PIPELINE_STATE) as f:
    state = json.load(f)

with open(EXP_PRIMARY) as f:
    exp_primary = json.load(f)
...
items30 = primary_result.get("verification_items", [])
for item in items30:
    print(f"  [{item.get('result','?')}] {item.get('check','')}")

with open(OUTPUT_BASELINE, "w") as f:
    json.dump(result, f, ensure_ascii=False, indent=2)
print(f"\n[BASELINE] Saved: {OUTPUT_BASELINE}")
