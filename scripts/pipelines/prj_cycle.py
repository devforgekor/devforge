#!/usr/bin/env python3
# Status: production
# Path: nightly_batch.sh / 1h_cycle.sh
"""
...
from lib.pipeline_common import (
    save, load_input, strip_code_fence, _extract_json, timestamp,
...
def _run_prj(state, tag, rubric_append):
    ...
    p_max = 4096
    proposer_result = call_one(P_MODEL, SYS_P + rubric_append,
                   state.build_context("prj_p"),
                   f"P_{tag}", max_tok=p_max)
    save(f"p_{tag}", tag, proposer_result)
    p_findings = (proposer_result or {}).get("result", {}).get("findings", [])

    # R
    r_max = 2048
    reflector_result = call_one(R_MODEL, SYS_R + rubric_append,
                   f"Proposer findings:\n{json.dumps(p_findings, ensure_ascii=False, indent=2)[:4000]}",
                   f"R_{tag}", max_tok=r_max)
    save(f"r_{tag}", tag, reflector_result)
    r_verdicts = (reflector_result or {}).get("result", {}).get("verdicts", [])

    # J
    j_max = 2048
    judge_result = call_one(J_MODEL, SYS_J + rubric_append,
    ...
    save(f"j_{tag}", tag, judge_result)

    j_res = (judge_result or {}).get("result", {})
    ...
    prj_result = {
        ...
        "p_elapsed_ms": (proposer_result or {}).get("elapsed_ms", 0),
        "r_elapsed_ms": (reflector_result or {}).get("elapsed_ms", 0),
        "j_elapsed_ms": (judge_result or {}).get("elapsed_ms", 0),
...
def run_round(round_num, with_rubric, resume_state_path=None):
    ...
    if resume_state_path:
        ...
        python_verified = state_data.get("python_verify", {})
        ...
        python_verified = python_verify(data, tag)
    ...
    if fb_count > 0:
        ...
        fb_v27 = call_one("Qwen27B", SYS_V27 + rubric_append, fb_v27_ctx, f"27B_{feedback_tag}")
