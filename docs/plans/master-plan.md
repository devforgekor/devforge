...
/opt/projects/server/
├── docs/
│   ├── plans/master-plan.md                          ← THIS FILE (unified plan, English)
│   ├── specs/system-design.yaml                        ← Architecture reference
│   ├── plans/phases.md                        ← Phase tracker (Phase 1 done, Phase 2 planned)
│   ├── plans/tasks.yaml                       ← Kanban task tracker
│   └── migration-insights.md ← Previous roadmap (archive reference)
├── scripts/
│   ├── lib/
...
## 9. Extract Pipeline — Future Upgrades

### 9.1 Faithfulness Check: Substring → Semantic/Embedding

Current `extract_pipeline.py` uses substring matching (`evidence.lower() in source.lower()`) in `_check_faithfulness()`. This catches verbatim hallucinations but misses near-miss paraphrases and generates false positives for valid paraphrases.

**Future plan**: Replace substring check with embedding similarity (e.g., pgvector cosine distance on the existing `turns.embedding` vector column). A fact is faithful if its embedding is within N distance from the source span's embedding. This allows semantic equivalence detection without requiring exact text match.

**Trigger**: When embedding infrastructure is stable and `turns.embedding` coverage is sufficient.

---

*Generated: 2026-05-18T13:35:00Z | Machine-readable | Updates via PR to docs/plans/master-plan.md*
