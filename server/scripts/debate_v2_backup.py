...
MODELS: Dict[str, Dict[str, Any]] = {
    ...
    "phi-mini-moe": {
        ...
        "system_prompt_support": True,
        "bench_load_s": 80, "bench_toks": 6.0,
        "cache_ram": 1024,
    },
    "selene-mini": {
        "filename": "selene-1-mini-llama-3.1-8b-q4_k_m.gguf",
        "port": 8082, "ctx": 2048, "threads": 4, "mlock": 0,
        "max_tokens": 1024, "temperature": 0.1,
        "system_prompt_support": True,
    ...
    "deepcoder-14b": {
        ...
        "bench_load_s": 200, "bench_toks": 2.1,
        "cache_ram": 1024,
    },
    "phi-4-14b": {
...
class DebateSession:
    ...
    def __init__(
        ...
        self.with_api = with_api      # use DeepSeek API for real search results
        ...
        if self.mode == "normal":
            ...
            self.refuter_model = "phi-mini-moe"
        ...
        # Neutral history summarizer (always on port 8081 — supervisor-managed, no separate server)
        self.summary_model = "phi-4-14b"
    ...
    def call_llm(self, messages: List[Dict], model_id: str) -> Optional[str]:
        ...
        Timeout is derived from max_tokens / expected_gen_rate + 300s buffer.
        14B models generate at ~1.4-2.1 tok/s, 32B at ~0.5 tok/s.
        A flat 300s was too short for 2048 max_tokens on 14B (needs ~25 min).
        """
        cfg = MODELS[model_id]
        port = cfg["port"]
        ...
        # Dynamic timeout: max_tokens / expected gen rate + prompt buffer
        gen_rate = cfg.get("bench_toks", 2.0)  # conservative default 2.0 tok/s
    ...
    def execute_search(self, queries: List[str]) -> List[Dict]:
        """Execute web searches. Uses DeepSeek API when with_api=True, stub otherwise."""
        results = []
        for q in queries:
            ...
            if self.with_api:
                print(f"  [search] DeepSeek API query: '{q}'...")
                from lib.llm.client import call_llm
                deepseek_key = os.getenv("DEEPSEEK_API_KEY", "")
                if not deepseek_key:
                    print(f"  [search] ERROR: DEEPSEEK_API_KEY not set")
                    results.append({"query": q, "results": [], "source": "error: no key"})
                    self.search_count += 1
                    self.search_queries.append(q)
                    self._save_state({
                        "type": "search_exec", "query": q,
                        "results_count": 0, "source": "error: no key",
                    })
                    continue
                status, body = call_llm(
                    ...
                    [{"role": "user", "content": (
                        ...
                        f"Provide factual information, relevant code patterns, "
                        f"documentation references, and common solutions. "
                ...
                if status == 200:
                    ...
                    print(f"  [search]   {len(content)} chars returned")
                    results.append({"query": q, "results": [{"content": content}], "source": "deepseek-api"})
                else:
                    err = body.get("error", "unknown")[:100]
                    print(f"  [search]   API error: {err}")
                    results.append({"query": q, "results": [], "source": f"api error: {err}"})
            else:
                print(f"  [search] query: '{q}'... (stub — use --with-api for real search)")
            ...
            self._save_state({
                "type": "search_exec", "query": q,
                "results_count": len(results[-1].get("results", [])),
                "source": results[-1].get("source", "stub"),
            })
        return results
...
def main():
    ...
    ap.add_argument("--with-api", action="store_true",
                    help="Use DeepSeek API for real search results (internet knowledge)")
    args = ap.parse_args()
