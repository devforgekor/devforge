...
def run_experiment(phases):
    ...
    # Stop watchdog + 15m cycle to prevent mode file / resource interference
    for unit in ["devforge-watchdog.service", "devforge-15m-cycle.service",
                  "devforge-15m-cycle.timer"]:
        subprocess.run(["systemctl", "--user", "stop", unit], capture_output=True, timeout=10)
    log("  Watchdog + 15m cycle stopped (experiment in progress)")
