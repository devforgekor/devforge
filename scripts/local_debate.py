...
review mode (v1.0, 2-person):
  ...
  Pod B (:8081): Qwen7B — Reviewer (Refuter + Judge combined)
...
class LocalDebate:
    ...
    def run_session(self) -> Optional[dict]:
        ...
        if not self.dry_run:
            subprocess.run(
                ["systemctl", "--user", "start", "container-devforge-swap.service"],
                capture_output=True)
...
class LocalDebateReview(LocalDebate):
    ...
    def _print_header(self) -> None:
        ...
        print("█ Pod B (:8081): Qwen7B — Reviewer (Refuter + Judge combined)")
        print(f"█ Question: {self.question[:80]}...")
        print(f"{'█'*60}")
