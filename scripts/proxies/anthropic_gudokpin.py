#!/usr/bin/env python3
# Status: production
# Path: systemd:anthropic-gudokpin-proxy
"""Anthropic-to-OpenAI format conversion proxy for Gudokpin.

  Claude Code → anthropic-gudokpin-proxy (:44779)
    → api.gudokpin.com/v1/chat/completions (GPT & Claude unified gateway)
...
# ── Config ─────────────────────────────────────────────────────────────────
DEFAULT_LISTEN = "127.0.0.1:44779"
...
class OpenRouterProxyHandler(BaseHTTPRequestHandler):
    ...
    def _forward_to_deepseek(
        ...
        Called when Gudokpin returns 5xx or connection fails.
        """
        print("[gudokpin-proxy] fallback → DeepSeek proxy :44777", file=sys.stderr)
        ...
        try:
            ...
            conn.close()
            print(f"[gudokpin-proxy] DeepSeek fallback also failed: {e}", file=sys.stderr)
    ...
    def _handle_messages(self, body: bytes) -> None:
        ...
        try:
            ...
            print(f"[gudokpin-proxy] conversion error: {e}", file=sys.stderr)
            self.send_response(400)
            self.send_header("Content-Type", "application/json")
            self.send_header("Content-Length", str(len(err)))
            self.end_headers()
        ...
        for key_idx, api_key in enumerate(API_KEYS):
            ...
            try:
                ...
                print(
                    f"[gudokpin-proxy] {key_label} upstream connection failed: {e}",
                    file=sys.stderr,
                )
                conn.close()
            ...
            if cache_age:
                print(f"[gudokpin-proxy] {key_label} cache={cache_state} age={cache_age}s", file=sys.stderr)
            else:
                print(f"[gudokpin-proxy] {key_label} cache={cache_state}", file=sys.stderr)
            ...
            if resp.status in RETRY_KEY_STATUSES:
                ...
                print(
                    f"[gudokpin-proxy] {key_label} OpenRouter {resp.status} "
                    f"({err_body[:120]!r}), "
    ...
    def _nonstream_response(self, resp: http.client.HTTPResponse, anthropic_model: str) -> None:
        ...
        try:
            ...
            print(f"[gudokpin-proxy] response conversion error: {e}", file=sys.stderr)
            ...
            while b"\n" in buffer:
                ...
                if line.startswith(b"data: "):
                    ...
                    try:
                        ...
                        print(f"[gudokpin-proxy] stream conversion error: {e}", file=sys.stderr)
    ...
    def log_message(self, fmt: str, *args) -> None:
        print(f"[gudokpin-proxy] {self.address_string()} - {fmt % args}", file=sys.stderr)


def main() -> None:
    import argparse

    parser = argparse.ArgumentParser(description="Anthropic→Gudokpin format-conversion proxy")
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=f"Gudokpin endpoint (default: {DEFAULT_UPSTREAM})",
    )
    ...
    print(f"[gudokpin-proxy] listening on {host}:{port}", file=sys.stderr)
    print(f"[gudokpin-proxy] upstream: {args.upstream}", file=sys.stderr)

    try:
        server.serve_forever()
    except KeyboardInterrupt:
        print("[gudokpin-proxy] shutting down", file=sys.stderr)
        server.server_close()
