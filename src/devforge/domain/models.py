...
class Turn(Base):
    ...
    __table_args__ = (
        ...
        Index("idx_turns_source_msg", "source_message_id", postgresql_where=sql_text("source_message_id IS NOT NULL"), unique=True),
...
class Observation(Base):
    ...
    __table_args__ = (
        ...
        Index("idx_observations_trgm", sql_text("observation gin_trgm_ops"), postgresql_using="gin"),
    )
...
class ObsDec(Base):
    ...
    id: Any = Column(PG_UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    turn_id = Column(PG_UUID(as_uuid=True), ForeignKey("turns.id", ondelete="CASCADE"), nullable=False, unique=True)
...
class MCPDec(Base):
    ...
    id: Any = Column(PG_UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    conversation_id = Column(PG_UUID(as_uuid=True), ForeignKey("conversations.id", ondelete="CASCADE"))
    summary = Column(Text)
    detail = Column(Text)
...
class Embedding(Base):
    ...
    __table_args__ = (
        Index("idx_embeddings_unique", "source_type", "source_id", "model_name", "chunk_index", unique=True),
...
class WorklogEntry(Base):
    ...
    __table_args__ = (
        ...
        Index("idx_worklog_one_in_progress", sql_text("(status)"), postgresql_where=sql_text("status = 'in_progress'"), unique=True),
    )
...
class ActivityLog(Base):
    ...
    __table_args__ = (
        ...
        Index("idx_activity_commit", "git_commit_hash", postgresql_where=sql_text("git_commit_hash IS NOT NULL")),
        ...
        Index("idx_activity_parent", "parent_id", postgresql_where=sql_text("parent_id IS NOT NULL")),
        Index("idx_activity_queue", "queue_status", sql_text("created_at"), postgresql_where=sql_text("queue_status = 'unprocessed'")),
        Index("idx_activity_commit_unique", "git_commit_hash", postgresql_where=sql_text("git_commit_hash IS NOT NULL AND type = 'commit'"), unique=True),
        Index("idx_activity_stage_unique", "run_id", "type", "parent_id", postgresql_where=sql_text("run_id IS NOT NULL AND type = 'stage' AND exec_status = 'DONE'"), unique=True),
    )
...
class FileRegistry(Base):
    ...
    __table_args__ = (
        ...
        Index("idx_file_registry_desc_trgm", sql_text("description gin_trgm_ops"), postgresql_using="gin"),
        Index("idx_file_registry_filename_trgm", sql_text("filename gin_trgm_ops"), postgresql_using="gin"),
...
class ReflexRule(Base):
    ...
    __table_args__ = (
        ...
        Index("idx_reflex_rules_pattern", sql_text("trigger_pattern gin_trgm_ops"), postgresql_using="gin"),
