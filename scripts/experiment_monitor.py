...
if os.path.exists(_sf):
    for line in open(_sf).read().split("\n"):
        ...
        if line and not line.startswith("#") and "=" in line:
            ...
            if k.strip() == "SLACK_BOT_TOKEN": SLACK_TOKEN = v
            elif k.strip() == "SLACK_CHANNEL": SLACK_CHANNEL = v
...
def get_llm_cpu():
    try:
        out = subprocess.check_output(["ps", "-eo", "pid,%cpu,%mem,args", "--sort=-%cpu"],
                                      timeout=5, text=True)
        for line in out.split("\n"):
            if "llama-server" in line and "port 8080" in line:
                pod_b = line.split()[1]
            if "llama-server" in line and "port 8082" in line:
                pod_a = line.split()[1]
        return pod_a, pod_b
    except: return "?", "?"
...
def check_phase0_progress():
    ...
    try:
        out = subprocess.check_output(
            ["find", "/opt/projects/server/data/eval", "-name", "*.json", "-mmin", "-90", "-type", "f"],
            timeout=5, text=True)
...
def build_status():
    ...
    if not alive:
        ...
        msg = (f"*[Experiment Monitor]* 실험 종료됨\n"
    ...
    try:
        out = subprocess.check_output(
            ["podman", "ps", "--filter", "name=devforge-pod", "--format", "{{.Names}} {{.Status}}"],
            timeout=5, text=True)
        containers = out.strip()
    except: pass

    phase_summary = f"Phase 0 (진행 중)" if current_phase == 0 else f"Phase {current_phase} 진행 중"
...
def main():
    ...
    while True:
        ...
        slack_send(msg)
        # 모든 Phase 완료 체크
        phases_completed = sum(1 for p in range(5) if os.path.exists(
            os.path.join(EXPER_DIR, f"phase{p}_metrics.json")))
