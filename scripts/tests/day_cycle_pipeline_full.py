...
Stages (matching day_cycle.sh exactly):
  ...
  3. FTS5 Refresh
  4. Extract (extract.py, PARALLEL=2)
  5. MCP Enrich
  6. Verify (3 prompt variants: OLD vs V2 vs V3_COT, VERIFY_PARALLEL=2)
  7. Embed (embed_batch.py, MAX_BATCH_SIZE=6)
...
def run_stage(
    cmd: list, timeout: int = 3600, label: str = "", extract_metrics: list = None
) -> dict:
...
def switch_inference(
    model_key: str, port: int, skip_probe: bool = False, timeout: int = 600
) -> dict:
    """Switch inference via pod_manager.start_inference. Returns success + switch time."""
    from lib.pod_manager import start_inference

    t0 = time.monotonic()
    try:
        ok = start_inference(model_key, port, skip_probe=skip_probe)
    except Exception as e:
        print(f"  [inference] switch to {model_key}:{port} failed: {e}", flush=True)
        ok = False
    elapsed = time.monotonic() - t0
...
def select_turns(limit: int = 10) -> list:
    ...
    print(
        f"  [select] {len(rows)} turns (min={min(sizes)}, max={max(sizes)}, "
        f"avg={sum(sizes) // len(sizes)} chars, {n_empty} empty text)",
        flush=True,
    )
    for r in rows:
...
def stage_text_clean(turn_ids: list) -> dict:
    ...
    print(f"\n{'=' * 50}", flush=True)
    print("  Stage 1/6: Text Preprocess (text_clean)", flush=True)
...
def stage_polish(limit: int) -> dict:
    """Polish + Self-Verify — matches day_cycle.sh polish stage, with retry on inference failure."""
    print(f"\n{'=' * 50}", flush=True)
    print("  Stage 2/6: Polish + Self-Verify (4B Q8 :8082, PARALLEL=2)", flush=True)
    print(f"{'=' * 50}", flush=True)
    ...
    for attempt in range(1, max_retries + 1):
        ...
        if attempt > 1:
            ...
            # Check inference — if down, switch to polish-4b
            try:
                urllib.request
                ...
                print("  [retry] inference :8082 down — re-switching to polish-4b", flush=True)
                sw = switch_inference("polish-4b", 8082)
                if not sw["ok"]:
                    print("  [error] polish-4b re-switch failed — abort", flush=True)
                    return {"ok": False, "elapsed_s": 0, "exit": -1, "metrics": {}}
                print(f"  [retry] inference ready after {sw['elapsed_s']}s", flush=True)
        ...
        if attempt < max_retries:
            print(
                f"  [retry] Polish failed (attempt {attempt}) — inference may have restarted",
                flush=True,
            )
            time.sleep(10)
...
def stage_fts5() -> dict:
    ...
    print(f"\n{'=' * 50}", flush=True)
    print("  Stage 3/6: FTS5 Refresh", flush=True)
    print(f"{'=' * 50}", flush=True)
    test_heartbeat("fts5")
    return run_stage(
...
def stage_embed(limit: int) -> dict:
    """Embed — switch inference to embed mode, run embed_batch.py, switch back."""
    print(f"\n{'=' * 50}", flush=True)
    print("  Stage 4/6: Embed (8B f16 :8081, MAX_BATCH_SIZE=6)", flush=True)
    print(f"{'=' * 50}", flush=True)
    test_heartbeat("embed_switch")

    # Switch inference to embed mode (:8081)
    sw = switch_inference("embed", 8081, skip_probe=True)
    print(
        f"  [inference] embed mode: {'OK' if sw['ok'] else 'FAIL'} ({sw['elapsed_s']}s)", flush=True
    )
    ...
    for i in range(120):
        ...
        print("  [embed] Not ready after 600s — skip", flush=True)
        switch_inference("extractor", 8082, skip_probe=True)
    ...
    result = run_stage(
        [
            sys.executable,
            "-u",
            os.path.join(PIPELINES_DIR, "embed_batch.py"),
            "--limit",
            str(limit),
        ],
        timeout=3600,
    ...
    print("  [inference] Switching back to extractor mode (:8082)...", flush=True)
    switch_inference("extractor", 8082, skip_probe=True)
    return result
...
def stage_extract(limit: int) -> dict:
    ...
    print(f"\n{'=' * 50}", flush=True)
    print("  Stage 5/6: Extract (7B Q8 :8082, PARALLEL=2)", flush=True)
    print(f"{'=' * 50}", flush=True)
    test_heartbeat("extract")

    # Check inference reranker (day_cycle.sh does ensure_pod_a "reranker")
    ...
    try:
        ...
        if resp.status == 200:
            print(f"  [reranker] inference :{reranker_port} healthy", flush=True)
        else:
            print(
                f"  [reranker] inference :{reranker_port} not healthy — no reranker verdicts",
                flush=True,
            )
    except Exception:
        print(
            f"  [reranker] inference :{reranker_port} unreachable — no reranker verdicts",
            flush=True,
        )
    ...
    for i in range(60):
        ...
        print("  [extractor] :8082 not ready after 180s — proceeding anyway", flush=True)
...
def stage_enrich(limit: int) -> dict:
    ...
    print(f"\n{'=' * 50}", flush=True)
    print("  Stage 6/6: Enrich (7B Q8 :8082)", flush=True)
    print(f"{'=' * 50}", flush=True)
...
def run_verify(snapshot_path: str, system_prompt: str, label: str, chunk_size: int = 6) -> dict:
    """Run verify on snapshot data with given prompt. chunk_size=6 matches day_verify.py."""
    print(f"\n{'=' * 50}", flush=True)
    print(
        f"  [VERIFY:{label}] on {os.path.basename(snapshot_path)} (chunk={chunk_size})", flush=True
    )
    print(f"{'=' * 50}", flush=True)
    t0 = time.monotonic()
    ...
    print(
        f"  [load] {len(findings)} findings → {len(chunks)} chunks (chunk={chunk_size})", flush=True
    )
    ...
    with ThreadPoolExecutor(max_workers=min(2, len(chunks))) as executor:
        futures = {
            executor.submit(verify_chunk, ch, ci + 1, len(chunks), system_prompt): ci
            for ci, ch in enumerate(chunks)
        }
        for f in as_completed(futures):
            r = f.result()
            merged_items.extend(r.get("items", []))
            for k, v in r.get("grounding_dist", {}).items():
                gdist[k] = gdist.get(k, 0) + v
            for k, v in r.get("result_dist", {}).items():
                rdist[k] = rdist.get(k, 0) + v
    ...
    print(
        f"  [{label}] {result['items_verified']} items, G={result['grounding_dist']}, "
        f"R={result['result_dist']} ({elapsed:.0f}s)",
        flush=True,
    )
...
def print_verify_comparison(results: list):
    ...
    print(f"\n{'=' * 70}", flush=True)
    print("  VERIFY PROMPT COMPARISON", flush=True)
...
def main():
    ...
    parser.add_argument(
        "--snapshot", type=str, default="", help="Skip Phase 1-2, load existing snapshot"
    )
    ...
    TEST = test_setup("day_cycle_full", f"전체 day_cycle 파이프라인 검증 ({args.limit} turns)")
    ...
    # Ensure inference reranker is up and mode file is set — prevents kill_all()
    # from stopping inference when inference switches modes
    from lib.pod_manager import start_inference

    print("  [inference] Ensuring reranker mode...", flush=True)
    start_inference("reranker", 8080)
    ...
    if not snapshot_path:
        ...
        print(
            "  Text Clean → Polish (PARALLEL=2) → FTS5 → Embed (BATCH=6) → Extract (PARALLEL=2) → MCP",
            flush=True,
        )
        print("=" * 60, flush=True)
        ...
        pipeline_stages.append(
            {
                ...
                "metrics": {"n_cleaned": str(r1.get("n_cleaned", 0))},
            }
        )
        snapshot_path = save_snapshot("01_after_clean", turn_ids)

        # Stage 2: Polish — ensure fresh inference
        print("  [inference] Switching to polish-4b mode (:8082)...", flush=True)
        sw = switch_inference("polish-4b", 8082)
        if not sw["ok"]:
            print("  [error] polish-4b mode failed — abort", flush=True)
            test_complete("error: polish-4b not ready")
            return
        print(f"  [inference] polish-4b ready ({sw['elapsed_s']}s)", flush=True)
        print("  [inference] settling 15s for full model init...", flush=True)
        time.sleep(15)
        ...
        r3 = stage_fts5()
        pipeline_stages.append(
            {"name": "3_fts5", "ok": r3["ok"], "elapsed_s": r3["elapsed_s"], "metrics": {}}
        )

        # Stage 4: Extract
        r4 = stage_extract(args.limit)
        pipeline_stages.append(
            {
                "name": "4_extract",
                "ok": r4["ok"],
        ...
        snap4 = save_snapshot("04_after_extract", turn_ids)

        # Stage 5: Enrich
        r5 = stage_enrich(args.limit)
        pipeline_stages.append(
            {
                "name": "5_enrich",
                "ok": r5["ok"],
        ...
        snapshot_path = save_snapshot("05_after_enrich", turn_ids, f"n{args.limit}")
        print(f"\n  [phase1] Snapshot ready: {snapshot_path}", flush=True)

    # ── Parallel/Batching verification ──
    print(f"\n{'=' * 60}", flush=True)
    print("  PARALLEL & BATCHING VERIFICATION", flush=True)
    print(f"{'=' * 60}", flush=True)
    ...
    # Switch inference to 14B verify mode
    print("  Switching inference to verify-14b mode (:8083)...", flush=True)
    sw = switch_inference("verify-14b", 8083)
    if not sw["ok"]:
    ...
    # ── Cleanup ──
    print("\n  Restoring inference to day mode...", flush=True)
    switch_inference("extractor", 8082, skip_probe=True)
