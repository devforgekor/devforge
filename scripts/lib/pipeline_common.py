...
EXPER_DIR = os.path.join(SCRIPTS_DIR, "..", "data", "experiment")   # → /opt/projects/server/data/experiment/
PIPELINE_DIR = os.path.join(SCRIPTS_DIR, "..", "data", "pipeline_run")  # → /opt/projects/server/data/pipeline_run/
EVENTS_DIR = os.path.join(PIPELINE_DIR, "events")                   # → /opt/projects/server/data/pipeline_run/events/
...
def save(phase, tag, data):
    ...
    fpath = os.path.join(PIPELINE_DIR, fname)  # → data/pipeline_run/
...
def llm_call(messages, model, max_tokens=2048, label=""):
    """LLM 호출 → JSON 파싱 with self-correction."""
    raw_content = ""
    def _try(m):
        nonlocal _raw
