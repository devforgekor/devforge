...
from lib.watchdog.config import (
    SANDBOX_IMAGE, SANDBOX_TIMEOUT, SANDBOX_MEM_LIMIT,
)
...
def _sandbox_verify(code_patch: dict) -> dict:
    ...
    # Build a test script from the patch
    test_code = "import sys\n"
    for f in code_patch.get("files", []):
        test_code += f"\n# Verify: {f['path']}\n"
        # Basic syntax validation via compile
...
def run_fix_loop(
    ...
    """
    result = {"fixed": False, "attempts": 0, "detail": ""}
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        if not sandbox["ok"]:
            log(f"  sandbox verify FAILED (exit {sandbox['exit_code']})")
