...
- **day-cycle 의도적 정지(2026-09-23)**: `devforge-day-cycle`은 critical(`SERVICE_TARGETS`)이라 down 시 watchdog이 재시작한다. `~/.config/devforge/day-cycle.paused` 플래그가 있으면 auto-start·day fix loop·**critical-service 복구**를 모두 건너뛴다(`_day_cycle_paused()`). 정지 절차는 [daily-server-maintenance §2.3](./runbooks/daily-server-maintenance.md).
