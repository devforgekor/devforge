#!/bin/bash
# day_verify.sh — Day verify cycle (:30)
# Chain: fast(code-struct → duckdns → worklog) → heavy(day_verify.py)
# Pod B (14B) only. MODE=night → skip.
set -o pipefail
MAX_CYCLE_SEC=1500  # 25분
START_TS=$(date +%s)
LOG_TS() { date -u +"%Y-%m-%dT%H:%M:%SZ"; }

echo "[$(LOG_TS)] day_verify start"

# ── Night window guard ───────────────────────────────────────────────
MODE_FILE="/opt/ai_data/scripts/current-system-mode.env"
if [ -f "$MODE_FILE" ] && grep -q "MODE=night" "$MODE_FILE"; then
    echo "[$(LOG_TS)] day_verify skipped (MODE=night)"
    exit 0
fi
...
if [ $BUDGET -le 60 ]; then
    ...
    echo "[$(LOG_TS)] === Heavy: day_verify (14B verify + category, Pod B) ==="
    timeout -k 10 "$BUDGET" python3 /opt/projects/server/scripts/pipelines/day_verify.py 2>&1
    RC=$?
    if [ $RC -eq 124 ]; then
...
echo "[$(LOG_TS)] day_verify complete (${TOTAL}s)"
