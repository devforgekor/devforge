#!/usr/bin/env python3
# Status: experimental
# Path: tests/test_handoff_quality.py — pytest
...
if __name__ == "__main__":
    ...
    if do_eval:
        port = MODEL_REGISTRY.get("verifier", {}).get("port", 8081)
        ok = start_pod_b("verify", port)
        if ok:
            try:
                log("  Calling 27B evaluator...")
                resp = call_llm(eval_messages, model="verifier",
        ...
        if not eval_result:
            port = MODEL_REGISTRY.get("proposer", {}).get("port", 8080)
            ok = start_pod_b("review-p", port)
            if ok:
                try:
                    log("  Falling back to 30B evaluator...")
                    resp = call_llm(eval_messages, model="proposer",
                                    max_tokens=1024, timeout=TIMEOUT,
                                    json_mode=True, return_meta=True)
                    content = resp["content"] if isinstance(resp, dict) else resp
                    eval_result = parse_llm_json(content)
