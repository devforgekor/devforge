...
Usage:
  ...
  python3 slack.py --daemon     # same (default)
"""
...
def _verify_slack_signature(body: bytes, timestamp: str, signature: str) -> bool:
    ...
    if not SLACK_SIGNING_SECRET:
        print("[slack] WARNING: no SLACK_SIGNING_SECRET configured, skipping verification", file=sys.stderr, flush=True)
        return True
    ...
    try:
        with urllib.request.urlopen(req, timeout=10) as resp:
            ...
            if not result.get("ok"):
                print(f"[slack] slack API error: {result.get('error', '?')}", file=sys.stderr, flush=True)
                return False
            return True
...
class SlackHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if payload.get("type") == "url_verification":
            ...
            print(f"[slack] url_verification ok (challenge={challenge[:20]}...)", file=sys.stderr, flush=True)
            return
...
def main():
    ...
    print(f"[slack] listening on {host}:{port}", file=sys.stderr, flush=True)
    try:
