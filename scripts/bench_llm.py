    ...
    for i in range(runs):
        ...
        try:
            with urllib.request.urlopen(req, timeout=300) as resp:
                ...
                print(
                    f"  Run {i + 1}/{runs}: decode {predicted_per_s:.2f} t/s  "
...
def auto_discover_config(port: int) -> dict:
    ...
    config = {"port": port}
    # All ports on single devforge-inference container
    config["component"] = "inference"
    config["container"] = "devforge-inference"
    if port == 8080:
        config["mode"] = "reranker"
    elif port == 8081:
        config["mode"] = "embed"
    elif port == 8082:
        config["mode"] = "day"
    ...
    row = psql(
        f"SELECT config FROM active_config WHERE component = '{esc_sql(config.get('component', ''))}'"
    )
    if row:
        try:
...
def main():
    ...
    parser.add_argument(
        "--n-predict", type=int, default=100, help="Tokens to predict per run (default: 100)"
    )
    ...
    parser.add_argument(
        "--register", action="store_true", help="Register results in experiment_registry"
    )
    ...
    parser.add_argument(
        "--verdict",
        choices=["optimal", "accepted", "rejected", "baseline", "pending"],
        default="pending",
        help="Experiment verdict (default: pending)",
    )
    ...
    if results["stddev"] > 0:
        print(
            f"  Stddev: {results['stddev']:.3f}  (min: {results['min_decode']:.2f}, max: {results['max_decode']:.2f})"
        )
