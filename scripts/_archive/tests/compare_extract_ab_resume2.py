...
TURNS = [
    ...
    {"label": "mixed_long_12k",
     "user_turn": "첫 번째로 inference와 inference의 협력 구조를 개선해야 합니다. 현재는 inference가 reranker로만 사용되는데, MoE 구조를 도입하면 좋겠습니다. 두 번째로 PostgreSQL 쿼리 최적화가 필요합니다. 세 번째로 watchog heartbeat를 개선해야 합니다. 네 번째로 Caddy 리버스 프록시 설정을 검토해야 합니다. 다섯 번째로 백업 전략을 재수립해야 합니다.",
